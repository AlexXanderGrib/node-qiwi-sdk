 ...
 * @extends {WalletApi}
 */
class WalletFundingSourcesApi extends WalletApi {
    /**
     * ## Список балансов
     *
     ...
     */
    async getAccounts() {
        const { accounts } = await this.http.get(`funding-sources/v2/persons/${this.walletId}/accounts`);
        return accounts;
    }
    /**
     * ## Создание баланса
    ...
    async getAccountOffers() {
        return await this.http.get(`funding-sources/v2/persons/${this.walletId}/accounts/offer`);
    }
    /**
    ...
    async setDefaultAccount(alias) {
        // Деструктивный метод
        /* istanbul ignore next */
        return await this.http.patch(`funding-sources/v2/persons/${this.walletId}/accounts/${alias}`, { defaultAccount: true });
    }
}

export { WalletFundingSourcesApi };
