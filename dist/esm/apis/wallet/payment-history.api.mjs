...
class WalletPaymentHistoryApi extends WalletApi {
    ...
    async getHistory(parameters) {
        return await this.http.get(`payment-history/v2/persons/${this.walletId}/payments?${formatQuerystring(parameters)}`);
    }
    /**
     * ## Статистика платежей
     *
    ...
    async getTotal(parameters) {
        return await this.http.get(`payment-history/v2/persons/${this.walletId}/payments/total?${formatQuerystring(parameters)}`);
    }
    /**
    ...
    async getTransaction(transactionId, type) {
        return await this.http.get(`payment-history/v2/transactions/${transactionId}?${formatQuerystring({
            type
        })}`);
    }
    /**
    ...
    async sendTransactionCheque(transactionId, type, email) {
        ...
        await this.http.post(`payment-history/v2/transactions/${transactionId}/cheque/send?${formatQuerystring({
            type
        })}`, { email });
    }
}
