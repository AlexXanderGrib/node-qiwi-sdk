 ...
 * @extends {Wallet}
 */
class _WalletCompat extends wallet.Wallet {
    /**
     ...
     */
    async getPaymentHistory(parameters) {
     ...
     */
    async getPaymentHistoryTotal(parameters) {
    ...
    async getTransactionCheque(transactionId, type, format = wallet_types.ChequeFormat.JPEG) {
        return await this.paymentHistory.getTransactionCheque(transactionId, type, format);
    }
    /**
     *
     * @param {types.StringOrNumber} transactionId  номер транзакции из {@link https://developer.qiwi.com/ru/qiwi-wallet-personal/#history_data|истории платежей} (параметр data[].txnId в ответе)
     ...
     */
    async pay(provider, account, amount, currency = _WalletCompat.Currency.RUB, fields = {}, comment = "") {
     ...
     * @return {Promise<types.PaymentResponse>}
     */
    async pay2({ account, amount, provider = _WalletCompat.Recipients.QIWI, comment = "", currency = _WalletCompat.Currency.RUB, fields = {} }) {
     ...
     */
    async getCardRequisites(cardId) {
...
_WalletCompat.IdentificationLevel = wallet_types.PersonIdentificationLevel;
_WalletCompat.ReceiptFormat = wallet_types.ChequeFormat;

exports._WalletCompat = _WalletCompat;
