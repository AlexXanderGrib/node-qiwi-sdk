...
class WalletWebhooksApi extends api.WalletApi {
    ...
    async add(parameter, txnType) {
        const hookResponse = await this.http.put(`payment-notifier/v1/hooks?${querystring.formatQuerystring({
            hookType: 1,
            param: parameter,
            txnType
        })}`);
    ...
    async updateSecret(hookId = this.activeId) {
        const { key } = await this.http.post(`payment-notifier/v1/hooks/${hookId}/newkey`);
        this.keys.set(hookId, key);
        return key;
    }
    /**
    ...
    async checkSign(transaction) {
        ...
        p.split(".").reduce((p, c) => {
            ...
            .join("|");
        const hash2 = crypto.createHmac("sha256", Buffer.from(this.keys.get(hookId), "base64"))
            .update(signPayload)
            .digest();
        return crypto.timingSafeEqual(Buffer.from(hash), hash2);
    }
}
