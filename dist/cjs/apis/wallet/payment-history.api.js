...
class WalletPaymentHistoryApi extends api.WalletApi {
  ...
  async getHistory(parameters) {
    return await this.http.get(
      `payment-history/v2/persons/${
        this.walletId
      }/payments?${querystring.formatQuerystring(parameters)}`
    );
  }
  /**
   * ## Статистика платежей
  ...
  async getTotal(parameters) {
    return await this.http.get(
      `payment-history/v2/persons/${
        this.walletId
      }/payments/total?${querystring.formatQuerystring(parameters)}`
    );
  }
  /**
  ...
  async getTransaction(transactionId, type) {
    return await this.http.get(
      `payment-history/v2/transactions/${transactionId}?${querystring.formatQuerystring(
        {
          type
        }
      )}`
  ...
  async getTransactionCheque(
    transactionId,
    type,
    format = wallet_types.ChequeFormat.JPEG
  ) {
  ...
  async sendTransactionCheque(transactionId, type, email) {
    await this.http.post(
      `payment-history/v2/transactions/${transactionId}/cheque/send?${querystring.formatQuerystring(
        {
          type
        }
      )}`,
