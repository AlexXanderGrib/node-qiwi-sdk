...
var detector = require('../detector/detector.js');
var detector_errors = require('../detector/detector.errors.js');
...
class WalletPaymentsApi extends api.WalletApi {
     ...
     */
    async getCommission(provider, account, amount, { accountCurrency = wallet_types.Currency.RUB, paymentCurrency = wallet_types.Currency.RUB } = {}) {
    ...
    static createFormUrl(provider, options = {}) {
        ...
        return `https://qiwi.com/payment/form/${provider}?${querystring.formatQuerystring(data)}`;
    }
    /**
     * Данный метод создаёт ссылку на предзаполненную форму оплаты
     *
     ...
     * @memberof WalletPaymentsApi
     */
    async pay({ account, amount, provider = wallet_types.Recipients.QIWI, comment = "", currency = wallet_types.Currency.RUB, fields = {}, accountCurrency = wallet_types.Currency.RUB }) {
     ...
     * @memberof WalletPaymentsApi
     */
    async quickPay({ account, amount, commissionPayer = wallet_types.CommissionPayer.SENDER, provider = wallet_types.Recipients.QIWI, comment = "", currency = wallet_types.Currency.RUB, fields = {}, accountCurrency = wallet_types.Currency.RUB }) {
